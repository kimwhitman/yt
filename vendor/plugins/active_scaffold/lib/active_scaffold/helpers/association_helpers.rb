  ...
  module Helpers
    module AssociationHelpers
      ...
      def options_for_association(association, include_all = false)
        ...
        available_records ||= []
        # MODIFICATION BY ARRON
        # To support has_and_belongs_to_many on the same object (Video habtm Video)
        if @record && association.klass == @record.class
          available_records.delete_if { |model| model.id == @record.id }
        end
