...
module RestClient
	...
	class Request
    # MODIFICATIONS BY ARRON TO SUPPORT DIGEST: added "auth_type"
		attr_reader :method, :url, :payload, :headers, :user, :password, :auth_type
		...
		def initialize(args)
      # MODIFICATIONS BY ARRON TO SUPPORT DIGEST: "assigned auth_Type"
      ...
      @auth_type = args[:auth_type] || 'basic'
		end
		...
		def transmit(uri, req, payload)
			...
			net.start do |http|
        setup_credentials(uri, http, req)
		...
		def setup_credentials(uri, http, req)
      # MODIFICATIONS BY ARRON TO SUPPORT DIGEST AUTH
      # SEE CONFIG/INITIALIZERS/NET_HTTP_PATCH.RB
      return unless user
      if auth_type == 'basic'
		...
		def process_result(res)
			if %w(200 201 202 203 204).include? res.code
				decode(res['content-encoding'], res.body) unless res.code == "204"
		...
		def response_log(res)
			"# => #{res.code} #{res.class.to_s.gsub(/^Net::HTTP/, '')} | #{(res['Content-type'] || '').gsub(/;.*$/, '')} #{res.body.nil?? 0 : res.body.size} bytes"
		end
